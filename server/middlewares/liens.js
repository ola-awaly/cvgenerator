...
exports.checkRightsOne = async (req, res, next) => {
    ...
    {
        const users = await db.sequelize.query("SELECT users.id FROM liens inner join sections on sections.id=liens.section_id inner join cvs on cvs.id=sections.cv_id inner join users on users.id=cvs.user_id where liens.id="+req.params.id, { type: QueryTypes.SELECT });
       console.log(users);
        if(users.length )
        {
