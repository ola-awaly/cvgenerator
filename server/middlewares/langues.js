...
exports.checkRightsOne = async (req, res, next) => {
    ...
    {
        const users = await db.sequelize.query("SELECT users.id FROM langues inner join sections on sections.id=langues.section_id inner join cvs on cvs.id=sections.cv_id inner join users on users.id=cvs.user_id where langues.id="+req.params.id, { type: QueryTypes.SELECT });
