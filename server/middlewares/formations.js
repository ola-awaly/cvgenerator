...
exports.checkRightsOne = async (req, res, next) => {
    ...
    {
        const users = await db.sequelize.query("SELECT users.id FROM formations inner join sections on sections.id=formations.section_id inner join cvs on cvs.id=sections.cv_id inner join users on users.id=cvs.user_id where formations.id="+req.params.id, { type: QueryTypes.SELECT });
