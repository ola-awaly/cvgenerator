...
const RSA_PUBLIC_KEY = process.env.JWTPUBLICKEY;
...
exports.checkLogged = async (req, res, next) => {
	...
	jwt.verify(token, RSA_PUBLIC_KEY, (err, decoded) => {
		...
		userRepository.getOne(req.loggeduserid).then((loggeduser) => {
			//console.log(loggeduser);
			if (loggeduser) next();
			// au cas ou l'utilisateur loggé est supprimé entre temps
			else
				return res.status(401).send({
					message: 'Unauthorized!',
				});
...
exports.checkRights = async (req, res, next) => {
	if (req.loggeduserrole == 'admin') next();
