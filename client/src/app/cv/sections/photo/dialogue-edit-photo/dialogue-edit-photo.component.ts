...
export class DialogueEditPhotoComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data['cvid']) this.CvId = this.data['cvid'];
    else this.CvId = JSON.parse(localStorage.getItem('cvencours'));
    ...
    if (this.image) {
      ...
      this._cvapi.updatephoto(this.CvId, fd).subscribe({
        next: (data) => {
          ...
          this.imagePreview = 'http://localhost/api' + data['imageUrl'];
        },
        error: (err) => {
          console.log(err);
        },
  ...
  envoyerImg() {
    ...
    if (this.cropImgPreview) {
      ...
      this._cvapi
        .updatephoto64(this.CvId, { image: this.cropImgPreview })
        .subscribe({
          next: (data) => {
