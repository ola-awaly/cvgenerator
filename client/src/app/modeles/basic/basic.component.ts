...
import {
  Position,
  SectionPositionList,
} from 'src/app/models/SectionByPositionList.model';
...
export class BasicComponent implements OnInit {
  ...
  exportHtmlToPDF(destination = 1) {
    ...
    html2canvas(data, { allowTaint: true, useCORS: true }).then(
      async (canvas) => {
        let docWidth = 208;
        ...
        switch (destination) {
          case 1: //save to disk
            let p = await doc.save(this.cv.label + '.pdf', {
              returnPromise: true,
            });
  ...
  sendByMail() {
    let dialogRefSendByMail = this._dialog.open(
  ...
  save() {
    ...
    this._cvapi.update(this.cvid, this.cv).subscribe({
      next: (data) => {
        console.log(data);
        this._toasterService.success('Bien sauvegardé', 'Sauvegardé', {
  ...
  openPhotoDialog(): void {
    ...
    dialogRefPhoto.afterClosed().subscribe((result) => {
      console.log('The dialog was closed');
      console.log(result);
      if (result) this.cv.photo = result;
      // this.user = result;
  ...
  openProfileDialog(): void {
    ...
    dialogRefProfile.afterClosed().subscribe((result) => {
      ...
      console.log(result);
      if (result) {
        for (let i in result) this.cv[i] = result[i];
      }
  ...
  openSection(id: number, type: string) {
    ...
    switch (type) {
      case 'experience':
        let dialogRefExperience = this._dialog.open(
        ...
        dialogRefExperience.afterClosed().subscribe((result) => {
          ...
          console.log(result);
          if (result) {
            if (result['ok']) this.refreshLists(2);
          }
        });
        break;
      case 'formation':
        let dialogRefFormation = this._dialog.open(
      ...
      case 'lien':
        ...
        break;
      case 'competence':
        let dialogRefCompetence = this._dialog.open(
  ...
  refreshLists(choice: number = 1) {
    // choice = 1 get originals, choice = 2 update only certain infos
      ...
      next: (data: SectionPositionList) => {
        if (choice == 1) {
          ...
          this.sectionsGauche.rows.map((el) => {
            ...
            el.ExperienceProfessionnelles.sort((a, b) =>
              a.ordre > b.ordre ? 1 : -1
            );
  ...
  updateLists(data: SectionPositionList) {
    this.sectionsGauche.rows = this.sectionsGauche.rows.map((el) => {
      ...
      if (row) {
        for (let att in el) {
          if (att != 'position' && att != 'ordre') el[att] = row[att];
        }
        el.ExperienceProfessionnelles.sort((a, b) =>
          a.ordre > b.ordre ? 1 : -1
        );
